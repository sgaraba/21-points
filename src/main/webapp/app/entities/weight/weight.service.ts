...
export class WeightService {
  ...
  protected convertDateFromClient(weight: IWeight): IWeight {
    const copy: IWeight = Object.assign({}, weight, {
      timestamp: weight.timestamp && weight.timestamp.isValid() ? weight.timestamp.toJSON() : undefined
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((weight: IWeight) => {
        weight.timestamp = weight.timestamp ? moment(weight.timestamp) : undefined;
      });
    }
    return res;
  }
